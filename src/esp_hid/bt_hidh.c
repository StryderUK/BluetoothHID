...
#include "private/bt_hidh.h"
//#if CONFIG_BT_HID_HOST_ENABLED
#include "private/esp_hidh_private.h"
#include <string.h>
#include <stdbool.h>
...
static void bta_hh_cb(tBTA_HH_EVT event, tBTA_HH *p_data)
{
    ...
    case BTA_HH_ADD_DEV_EVT: {
        ...
        if (dev->status == BTA_HH_OK) {
            ...
            event_callback(event_loop_handle, ESP_HIDH_EVENTS, ESP_HIDH_OPEN_EVENT, &p);
        } else {
    ...
    case BTA_HH_CLOSE_EVT: {
        ...
        event_callback(event_loop_handle, ESP_HIDH_EVENTS, ESP_HIDH_CLOSE_EVENT, &p);
    } break;
...
void bta_hh_co_data(uint8_t handle, uint8_t *p_rpt, uint16_t len, tBTA_HH_PROTO_MODE  mode, uint8_t sub_class, uint8_t country_code, esp_bd_addr_t bda, uint8_t app_id)
{
    ...
    //if (event_loop_handle) {
        ...
        if (report->report_type == ESP_HID_REPORT_TYPE_FEATURE) {
            ...
            event_callback(event_loop_handle, ESP_HIDH_EVENTS, ESP_HIDH_FEATURE_EVENT, &p);
        } else {
            p.input.dev = dev;
            ...
            event_callback(event_loop_handle, ESP_HIDH_EVENTS, ESP_HIDH_INPUT_EVENT, &p);
        }
    }
}
