...
#if BTA_HH_DEBUG
    APPL_TRACE_DEBUG("bta_hh_api_enable StryderUK ESP-IDF 4.2 port");
#endif
                          ...
                          UINT32 data, BT_HDR *pdata)
{
    ...
    if (sm_event != BTA_HH_INVALID_EVT &&
        ...
        bta_hh_hdl_event((BT_HDR*)p_buf);
        // Free memory (usually done by message system?)
        if(p_buf) {
